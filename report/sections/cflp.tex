...
indexed by $s \in S$ and let $D$ be the set of demand sites, indexed by $d \in D$. By $c_{s,d}$ we 
denote the cost associated with site s supplying a single unit of supply to demand site d and by 
$f_s$ we denote the setup cost associated with constructing site $s$.
By $u_s$ we denote the maximum capacity that site $s$ can supply overall and by $r_d$ we denote the demand 
...
\begin{subequations}\label{eq:CFLP}
    \begin{align}
    \text{CFLP: } \min & \sum_{s \in S} f_s y_s + \sum_{s \in S}\sum_{d \in D} c_{s,d} x_{s,d} \label{eq:CFLP_obj}\\
